...
func TestInteractiveModeSupportsCursorEditingAndControls(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "Ctrl-C cancels the current line",
			inputSteps: [][]byte{[]byte("discard"), []byte("\x03"), []byte("q\r")},
		},
		{
			name:       "Ctrl-D exits",
...
func runInTerminal(t *testing.T, binary, commandName string, inputSteps ...[]byte) string {
	...
	command := exec.Command("sh", "-c", `"$MOONCLI_TEST_BINARY" "$MOONCLI_TEST_COMMAND"; stty -a; printf '\nSHELL-READY\n'`)
	...
	beforeInput := readTerminalUntil(t, terminal, []byte("> "))
	for _, input := range inputSteps {
		if _, err := terminal.Write(input); err != nil {
			t.Fatalf("write terminal input after %q: %v", beforeInput, err)
		}
		time.Sleep(100 * time.Millisecond)
	}
	afterInput := readTerminalUntil(t, terminal, []byte("SHELL-READY"))

	return string(beforeInput) + string(afterInput)
}
